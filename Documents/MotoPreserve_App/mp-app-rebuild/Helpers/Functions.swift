...
func doDataBackup(vc:UIViewController, bikes:[FB_Bike]) {
    ...
    bikesRef.removeValue { (error, refer) in
        ...
        putBackupItems(vc:vc, bikes:bikes, ref:ref)
    }
//    putBackupItems(vc:vc, bikes:bikes, ref:ref)
    //TODO: make sure this happens when animation is done instead.
//    doRestoreData(view: vc, tempBikes: bikes)
     //
}

func putBackupItems(vc:UIViewController, bikes:[FB_Bike], ref:DatabaseReference) {
    guard let uid = Auth.auth().currentUser?.uid else { return }
...
//////
 func doRestoreData(view:UIViewController, tempBikes:[FB_Bike]) {
    ...
    userBikesRef.queryOrdered(byChild: "uniqueID").observeSingleEvent(of: .childAdded, with: { (snapshot) in
